  ...
  static setPlaceholders(): void {
    ...
    for (const form of forms)
      for (let j = 0; j < form.elements.length; j++) {
        ...
        if ((el instanceof HTMLInputElement && (el.type === 'text' || el.type === 'tel' || el.type === 'email' || el.type === 'url') || el instanceof HTMLTextAreaElement)) inps.push(el);
      }
    ...
    for (const inp of inps) {
      if (inp.placeholder === '') {
        if (inp instanceof HTMLInputElement) {
          ...
          else if (inp.type === 'email') inp.placeholder = 'Escreva aqui um e-mail.'
