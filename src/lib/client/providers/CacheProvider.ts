...
export default class CacheProvider implements Provider {
  ...
  #setupIdentifier(): void {
    ...
    if (!this.#element.id) this.#element.id = `${auto}_${count}`;
  }
  ...
  async #checkForPersisters(): Promise<boolean> {
    try {
      if (
        !(this.#element.id || ("name" in this.#element && this.#element.name))
      )
      ...
      if (!persistData)
        throw new ReferenceError(`Could not find persistence container`);
      return true;
    } catch (e) {
      console.error(
  ...
  #setupPersisters(els: HTMLElement[]): void {
    ...
    if (!(this.#element.id || ("name" in this.#element && this.#element.name)))
      throw new DOMException("Failed to setup identifier", "SyntaxError");
    const idf = this.#element.id || (this.#element as HTMLFormElement).name;
    const persisters: { [k: string]: string } = {};
    for (let i = 0; i < els.length; i++) {
      const el = els[i],
      ...
      try {
        ...
        persisters[elIdf] = value;
      } catch (e) {
        console.error(
          `Error setting up persistent entries:\n${(e as Error).message}`
        );
      }
    }
    console.log(persisters);
    sessionStorage.setItem(idf, JSON.stringify(persisters));
  }
