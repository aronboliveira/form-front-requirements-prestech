...
export default function Range(
  ...
  const min = 0,
    ...
    coloringStylesheet = useRef<CSSStyleSheet | undefined>(
      undefined
    ),
    ...
    handleCtxComponent = useCallback(
      (
        ...
        switch (props.id as rangeCtxId) {
          ...
          case "planning":
            return (
              <Planning lvl={lvl} controller={controller} />
            );
          case "spreadSheets":
            return (
              <Spreadsheets
    ...
    handleSlide = useCallback(
      (
      ...
      ): void => {
        {
          ...
          if (i === -1) return;
          const rule =
            coloringStylesheet.current.cssRules[i].cssText;
          if (
            !rule.match("scale") &&
            rule.match(":focus")
          ) {
  ...
  return (
    ...
    >
      ...
      <div>
        <span className={`${s.rangeNumRef} ${s.rangeMin}`}>
          {min}
        </span>
        <input
          ref={r}
